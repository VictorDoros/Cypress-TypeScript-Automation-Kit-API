...
class OrderFlow {
  ...
  createCartAddItemGetCartCreateOrder(env: Environment, token: Token, productName: string) {
      ...
      (productId) => {
        return CartFlow.createCartAndReturnId(env).then((cartId) => {
          ...
          .then(() => {
            ...
            this.orderServices.getOrder(env, token, orderId).then((getOrderResponse) => {
    ...
    createCartAddItemCreateOrderUpdateOrder(env: Environment, token: Token, productName: string, orderComment: string) {
    return ProductFlow.getListProductsAndReturnProductId(env, productName).then(
      (productId) => {
        return CartFlow.createCartAndReturnId(env).then((cartId) => {
          return this.cartServices.addItemToCart(env, cartId, productId)
          .then(() => {
            ...
            this.orderServices.updateOrder(env, token, orderId, orderComment).then((updateOrderResponse)=>{
      ...
      createCartAddItemCreateOrderDeleteOrder(env: Environment, token: Token, productName: string) {
    return ProductFlow.getListProductsAndReturnProductId(env, productName).then(
      (productId) => {
        return CartFlow.createCartAndReturnId(env).then((cartId) => {
          return this.cartServices.addItemToCart(env, cartId, productId)
          .then(() => {
            ...
            this.orderServices.deleteOrder(env, token, orderId).then((deleteOrderResponse) => {
