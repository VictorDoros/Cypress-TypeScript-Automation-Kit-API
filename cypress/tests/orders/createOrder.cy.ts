...
describe('Create Order', {}, () => {
  ...
  it('Should create a new order successfully and return status 201', () => {
    App.apiActions.orderFlow
      .createCartAddItemGetCartCreateOrder(env, token, 'Cream Cheese')
      .then(({ createOrderResponse, getOrderResponse }) => {
        TestHelpers.defineTheStep(
          'Confirm status code 201 when creating an order'
        ).then(() => {
        ...
        TestHelpers.defineTheStep(
          'Verify the order contains one item and has an empty comment'
        ).then(() => {
